...
public class SqlInjectionLesson9 extends AssignmentEndpoint {
    ...
    protected AttackResult injectableQueryIntegrity(String name, String auth_tan) {
        ...
        String query = "SELECT * FROM employees WHERE last_name = " + "?" + " AND auth_tan = '" + auth_tan + "'";
        try (Connection connection = dataSource.getConnection()) {
            try {
                PreparedStatement statement = connection.prepareStatement(query, TYPE_SCROLL_SENSITIVE,
                        CONCUR_UPDATABLE);
